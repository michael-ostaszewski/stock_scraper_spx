# aplikacja z optymalnym pobieraniem wycinków z bazy:
...
selected_date = st.sidebar.date_input("Date selector", value=last_date.date())
...
numeric_cols = [
    ...
    "Price", "Buy Recommendation", "Hold Recommendation", "Sell Recommendation",
    "Dividend yield", "Market cap clear"
]

filtered_data, invalid_vals = clean_numeric(filtered_data.copy(), numeric_cols)

prev_date = None
if unique_dates:
    earlier_dates = [d for d in unique_dates if d < selected_date_ts]
    if earlier_dates:
...
scoring = pd.DataFrame(columns=required_columns)
best_three = pd.DataFrame(columns=required_columns)
total_stocks = 0
...
if not best_three.empty and best_three.shape[0] >= 3:
...
if filtered_data.empty:
    ...
    if prev_date is not None and not daily_metrics.empty:
        prev_data = daily_metrics[daily_metrics["Date of record"] == prev_date]
        if not prev_data.empty:
            prev_row = prev_data.iloc[0]
            delta_low = med_low - prev_row["Low Forecast Percent"]
            delta_median = med_median - prev_row["Median Forecast Percent"]
            delta_high = med_high - prev_row["High Forecast Percent"]
...
].dropna(subset=["Date of record"]).round(2)

with performance_block("build_global_forecast_history_chart"):
    fig = px.line(
        df_time,
        x="Date of record",
...
if not filtered_data.empty:
    recommendation_data = filtered_data[
        [
            "Number of analysts",
            "Buy Recommendation",
            "Hold Recommendation",
    ...
    if not recommendation_data.empty:
        ...
        total_buy = (recommendation_data["Number of analysts"] * recommendation_data["Buy Recommendation"]).sum()
        total_hold = (recommendation_data["Number of analysts"] * recommendation_data["Hold Recommendation"]).sum()
        total_sell = (recommendation_data["Number of analysts"] * recommendation_data["Sell Recommendation"]).sum()
...
if not scoring.empty:
    ...
    if prev_date is not None:
        prev_filtered_data = load_day_snapshot(prev_date, data_version)
        prev_filtered_data, _ = clean_numeric(prev_filtered_data.copy(), numeric_cols)
        if all(col in prev_filtered_data.columns for col in history_needed):
            prev_scoring = prev_filtered_data[history_needed].sort_values(
    ...
    with performance_block("build_selected_stocks_bar_chart"):
        fig_bar = px.bar(
            displayed_stocks,
            x="Stock",
            y="Score",
...
if not scoring.empty:
    ...
    with performance_block("build_median_vs_pe_chart"):
...
if not scoring.empty:
    selected_stocks = scoring["Stock"].tolist()
else:
    selected_stocks = []

# 2. For the "All S&P 500" baseline, we now use daily market aggregates
df_time_all = daily_metrics[
    [
        "Date of record",
        "Low Forecast Percent",
        "Median Forecast Percent",
...
].dropna(subset=["Date of record"]).copy()

# 3. For the "Filtered Stocks" (top N), retrieve only the needed ticker history
df_time_scoring = load_selected_stocks_daily_metrics(tuple(selected_stocks), data_version)
...
if not df_time_scoring.empty:
    ...
    df_scoring_melt = pd.DataFrame(columns=df_all_melt.columns)

# Combine both sets
df_combined = pd.concat([df_all_melt, df_scoring_melt], ignore_index=True)
...
with performance_block("build_selected_history_chart"):
    if selected_forecast == "All forecasts":
        fig = px.line(
            round(df_combined, 2),
            x="Date of record",
...
filtered_data_notnull = filtered_data.dropna(subset=["Number of analysts"]).copy()
...
with performance_block("build_top_analysts_chart"):
    fig_bar2 = px.bar(
        top_analysts_sorted,
        x="Stock",
...
with col1:
    selected_dividend_sector = st.selectbox("Select a sector for the dividend yield chart:", options=sector_options)
...
if selected_dividend_sector != "All Sectors":
    div_data = div_data[div_data["Sector"] == selected_dividend_sector]
...
with performance_block("build_dividend_chart"):
    fig_div = px.bar(
        div_data_sorted,
        x="Stock",
        y="Dividend yield",
...
if filtered_data.empty or "Smart Score" not in filtered_data.columns:
    ...
    delta_avg = None
    if prev_date is not None and not daily_metrics.empty:
        prev_day_data = daily_metrics[daily_metrics["Date of record"] == prev_date]
        if not prev_day_data.empty:
            prev_avg = prev_day_data.iloc[0]["Smart Score"]
...
if "Smart Score" in daily_metrics.columns and "Date of record" in daily_metrics.columns:
    df_smart_trend = daily_metrics[["Date of record", "Smart Score"]].dropna(subset=["Date of record"])

    with performance_block("build_smart_score_trend_chart"):
...
if filtered_data.empty or not all(c in filtered_data.columns for c in needed_cols):
    ...
    smart_score_picker_data = filtered_data[needed_cols].copy()
    scores_clean = (
        pd.to_numeric(smart_score_picker_data["Smart Score"], errors="coerce")  # NaN dla nieparsowalnych
        .replace([np.inf, -np.inf], np.nan)  # usuń ±Inf
    )

    smart_score_picker_data["SmartScoreInt"] = scores_clean.round().astype("Int64")
    ...
    subset = smart_score_picker_data[smart_score_picker_data["SmartScoreInt"].isin(score_choice)]

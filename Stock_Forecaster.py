...
# działająca apka z bazą danych - ale z nieoptymalnym wykorzystaniem bazy (pobiera całą bazę na raz)
...
#     ) as conn:
#         df = pd.read_sql(
#             'select * from market.view_spx_dashboard order by "Date of record" asc, "Time of record" asc nulls last',
#             conn
#         )
#     return df
#
#
# # jeśli chcesz zostawić sobie tryb fallback na GitHuba, możesz to odkomentować:
# # @st.cache_data
...
# =========================
# 2. POBIERAMY OSTATNIĄ DATĘ
# =========================
@st.cache_data(ttl=300)
def get_last_date():
    with get_conn() as conn:
        df = pd.read_sql(
            'select max("Date of record") as max_date from market.view_spx_dashboard',
            conn
        )
    return pd.to_datetime(df.loc[0, "max_date"])

# =========================
# 3. DANE DLA JEDNEJ DATY (UI)
# =========================
@st.cache_data(ttl=300)
def load_day(date_val: pd.Timestamp):
    with get_conn() as conn:
        df = pd.read_sql(
            """
            select *
            from market.view_spx_dashboard
            where "Date of record" = %s
            """,
            conn,
            params=[date_val.date()]   # ważne: tylko ta data!
        )
    return df

# =========================
# 4. DANE HISTORYCZNE (TYLKO POTRZEBNE KOLUMNY)
# =========================
@st.cache_data(ttl=600)
def load_history_for_charts():
    with get_conn() as conn:
        df = pd.read_sql(
            """
            select
...
# 2) selector po dacie – ale domyślnie ostatnia:
selected_date = st.sidebar.date_input("Date selector", value=last_date)

# 3) pobierz dane TYLKO dla wybranej daty
filtered_data = load_day(pd.Timestamp(selected_date))

# 4) osobno – dane historyczne do wykresów czasowych
df_history = load_history_for_charts()


# Convert the date column if it exists
if "Date of record" in df_history.columns:
    df_history["Date of record"] = pd.to_datetime(df_history["Date of record"], errors='coerce')


# Date filter
if "Date of record" in df_history.columns:
    unique_dates = sorted(df_history["Date of record"].dropna().unique())
    filtered_history_for_charts = df_history[df_history["Date of record"] == pd.Timestamp(selected_date)]
else:
    filtered_history_for_charts = df_history
    st.sidebar.info("No 'Date of record' column - displaying all data.")
...
if filtered_data.empty:
    ...
    delta_low = delta_median = delta_high = None
    if prev_date is not None:
        prev_data = df_history[df_history["Date of record"] == prev_date]
        delta_low = med_low - prev_data["Low Forecast Percent"].median()
        delta_median = med_median - prev_data["Median Forecast Percent"].median()
        delta_high = med_high - prev_data["High Forecast Percent"].median()
...
# Line chart showing median forecasts over time (all S&P500)
df_time = (
    df_history.dropna(subset=["Date of record"])
      .groupby("Date of record")[["Low Forecast Percent", "Median Forecast Percent", "High Forecast Percent"]]
      .median()
      .round(2)
      .reset_index()
...
if not scoring.empty:
    ...
    if prev_date is not None:
        prev_filtered_data = df_history[df_history["Date of record"] == prev_date]
...
# 3. For the "Filtered Stocks" (top N), we retrieve all historical data for the same tickers
df_scoring_all = df_history[df_history["Stock"].isin(selected_stocks)].copy()
df_scoring_all = df_scoring_all[
    (df_scoring_all["Smart Score"] > 7) &
    (df_scoring_all["Score"] > 2) &
    (df_scoring_all["Low Forecast Percent"] > -5) &
...
df_fgi = (
    df_history.dropna(subset=["Date of record", "Fear & Greed Index"])
      .groupby("Date of record")["Fear & Greed Index"]
      .mean()
      .reset_index()
)
...
if filtered_data.empty or "Smart Score" not in filtered_data.columns:
    ...
    delta_avg = None
    if prev_date is not None:
        prev_day_data = df_history[df_history["Date of record"] == prev_date]
        prev_avg = pd.to_numeric(
            prev_day_data["Smart Score"], errors="coerce"
        ).mean()
        if pd.notna(prev_avg):
...
# ── 2 ▸ TREND LINE OVER TIME ────────────────────────────────────────────
# Calculate daily mean across the entire dataset
if "Smart Score" in df_history.columns and "Date of record" in df_history.columns:
    df_smart_trend = (
        df_history.dropna(subset=["Date of record"])
          .assign(**{"Smart Score": pd.to_numeric(df_history["Smart Score"], errors="coerce")})
          .groupby("Date of record")["Smart Score"]
          .mean()
          .reset_index()
    )
...
# 1) spróbuj użyć lekkiego df_history,
#    ale jeśli nie ma tam potrzebnych kolumn, dociągnij z bazy tylko to, co trzeba
needed_turtle_cols = [
    "Date of record",
    "Stock",
    "Sector",
...
has_all_cols = all(col in df_history.columns for col in needed_turtle_cols)

if not has_all_cols:
    # dociągamy sam „cięższy” wycinek tylko pod Turtle
    @st.cache_data(ttl=600)
    def load_history_for_turtle():
        with get_conn() as conn:
            df = pd.read_sql(
                """
                select
    ...
    df_turtle = load_history_for_turtle()
else:
    # możemy użyć istniejącego df_history
    df_turtle = df_history[needed_turtle_cols].copy()
    df_turtle["Date of record"] = pd.to_datetime(df_turtle["Date of record"], errors="coerce")

# jeżeli po tym dalej nie mamy tych kolumn – odpuść
if (
    df_turtle.empty
    or "1-day range" not in df_turtle.columns
    or "Price" not in df_turtle.columns
):
    st.info("Cannot compute signals – missing price-range data in the database.")
else:
    # ── 1 ▸ helper – parse 'low high' text into floats ──────────────────
    def split_range(r: str):
        if pd.isna(r):
    ...
    # ── 2 ▸ working copy + Low/High columns ────────────────────────────
    df_all = df_turtle.copy()
    df_all[["Low", "High"]] = (
        df_all["1-day range"].apply(split_range).apply(pd.Series)
    )

    # wyrzuć rekordy bez cen / zakresu / daty
    df_all = df_all.dropna(subset=["Low", "High", "Price", "Date of record"])

    # sortowanie po tickerze i dacie
    df_all = df_all.sort_values(["Stock", "Date of record"])
    ...
    # wczorajsze wartości kanałów + close
    df_all["High20_y"] = df_all.groupby("Stock")["High20"].shift(1)
    df_all["Low10_y"]  = df_all.groupby("Stock")["Low10"].shift(1)
    df_all["Close_y"]  = df_all.groupby("Stock")["Price"].shift(1)

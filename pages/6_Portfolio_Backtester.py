...
# początek starej sekcji bez opcji wykluczenia spółek
...
# koniec starej sekcji bez opcji wykluczenia spółek


# ────────────────────────────────────────────────────────────────────────────────
# Back-test – grid search + full dashboard  (ticker-exclusion enabled)
# ────────────────────────────────────────────────────────────────────────────────
import re, itertools, numpy as np, time
from math import sqrt
import pandas as pd
import plotly.express as px
import streamlit as st
...
def load_and_clean() -> pd.DataFrame:
    ...
    def fix_num(x):
        if pd.isna(x):
            return None
        x = re.sub(r"[^\d.]", "", str(x).replace(",", ""))
        if x.count(".") > 1:                       # "7.866.87" → "7866.87"
            parts = x.split(".")
            x = "".join(parts[:-1]) + "." + parts[-1]
        return x

    num_cols = [
        "Closing Price", "Low Forecast", "Smart Score",
        "Score", "Low Forecast Percent", "Number of analysts",
    ]
    for col in num_cols:
        df[col] = df[col].apply(fix_num).pipe(pd.to_numeric, errors="coerce")

    return df.dropna(subset=[
        "Closing Price", "Low Forecast",
        "Score", "Smart Score", "Low Forecast Percent"
    ])
...
daily_budget = st.sidebar.number_input("Daily budget (USD)", 10., 100_000., 100., 100.)
top_n        = st.sidebar.slider("Top-N per day", 1, 20, 3)
...
# ── NEW: exclude tickers ───────────────────────────────────────────────
all_tickers = sorted(df["Stock"].unique())
...
# │ 2. BACK-TEST ENGINE                      │
# ╰──────────────────────────────────────────╯
def run_backtest(p, excl):
    mask = df["Date of record"].between(pd.Timestamp(d_start), pd.Timestamp(d_end))
    if excl:
        mask &= ~df["Stock"].isin(excl)
    data = df[mask]
    ...
    for day in sorted(data["Date of record"].unique()):
        ...
        for pos in positions[:]:
            ...
            if not row.empty and row.iloc[0]["Closing Price"] >= row.iloc[0]["Low Forecast"] * p["exit_mult"]:
                ...
                cash += proceeds - fee
                trades.append(
                    [day, "SELL", pos["tick"], pos["shr"], row.iloc[0]["Closing Price"],
                     proceeds, fee, proceeds - fee]
                )
                positions.remove(pos)
        ...
        for _, r in pick.head(top_n).iterrows():
            ...
            cash  -= alloc + fee
            positions.append({"tick": r["Stock"], "shr": shares, "cost": r["Closing Price"]})
            trades.append([day, "BUY", r["Stock"], shares, r["Closing Price"],
                           alloc, fee, -(alloc + fee)])

        # MTM
        mkt_val = sum(
            (
                day_rows[day_rows["Stock"] == pos["tick"]]["Closing Price"].iloc[0]
                if not day_rows[day_rows["Stock"] == pos["tick"]].empty
                else pos["cost"]
            ) * pos["shr"] for pos in positions
        )
    ...
    port_map = {}
    for pos in positions:
        d = port_map.setdefault(pos["tick"], {"shr":0., "cost":0.})
        d["shr"]  += pos["shr"]
        d["cost"] += pos["shr"] * pos["cost"]

    last_rows = data[data["Date of record"] == equity.index[-1]].set_index("Stock")
    rows = []
    for t, v in port_map.items():
        avg = v["cost"] / v["shr"]
        px  = last_rows["Closing Price"].get(t, avg)
        mv  = v["shr"] * px
        un  = v["shr"] * (px - avg)
        rows.append([t, v["shr"], avg, px, mv, un])
    ...
    invested        = daily_budget * len(equity)
    unrealised_sum  = portfolio["Unrealised P/L"].sum() if not portfolio.empty else 0.0
    net_profit_incl = equity["Equity"].iloc[-1] - invested
    ...
    stats = dict(
        ...
        Sharpe=sharpe,
        MaxDD=mdd,
        TradesRaw=len(trades_df.groupby(["Date","Action","Ticker"])),
        Tickers=len(portfolio),
        SharesTotal=portfolio["Shares"].sum() if not portfolio.empty else 0,
        MaxExposure=equity["Positions"].max(),
        CurExposure=equity["Positions"].iloc[-1],
...
def show_dashboard(eq, trades_df, port, stats):
    ...
    f2.metric("Total return w/ unreal.",  f"{stats['TotRetWithUnreal']*100:,.2f}%")
    f3.metric("Annual return w/ unreal.", f"{stats['AnnWithUnreal']*100:,.2f}%")
    f4.empty()  # placeholder

    # Cash vs positions kumulacja ---------------------------------------
    expo = (
        eq.reset_index()
          .melt(id_vars="Date", value_vars=["Cash","Positions"],
                var_name="Component", value_name="USD")
    )
    fig = px.area(expo, x="Date", y="USD", color="Component",
                  title="Cash vs. Positions value")
    fig.update_traces(stackgroup="one")
    fig.add_scatter(
        x=eq.index,
        y=daily_budget * np.arange(1, len(eq)+1),
    ...
    st.subheader("Trade log (grouped)")
    st.dataframe(
        trades_agg[["Date","Action","Ticker","Shares","Price","Gross","Fee","Profit"]]
          .round(4),
        use_container_width=True
    )
...
if search_btn:
    ...
    for i, vals in enumerate(grid, 1):
        p = dict(zip(rngs.keys(), vals))
        eq, tr, port, stats = run_backtest(p, exclude_tickers)
        ret = stats["TotRet"]
        results.append({**p, "Return": ret})
        if ret > best_ret:
            best_ret, best_p = ret, p
    ...
    eq, tr, port, stats = run_backtest(best_p, exclude_tickers)
    show_dashboard(eq, tr, port, stats)
...
if run_btn:
    ...
    eq, tr, port, stats = run_backtest(params, exclude_tickers)
    show_dashboard(eq, tr, port, stats)
elif not search_btn:
    st.info("Set parameters and click a button to run.")

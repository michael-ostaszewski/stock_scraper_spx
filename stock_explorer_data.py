...
STOCK_EXPLORER_VIEW_QUERY = text(
    """
    select *
    from market.view_stock_explorer_history
    where "Stock" = :ticker
    order by "Date of record" asc
    """
)

STOCK_EXPLORER_BASE_QUERY = text(
    """
    select
        stock as "Stock",
        price as "Price",
...
def _read_company_history(query, ticker: str, label: str) -> pd.DataFrame:
    started_at = time.perf_counter()
    with get_engine().connect() as conn:
        df = pd.read_sql_query(query, conn, params={"ticker": ticker})
    _log_timing(label, started_at)
    log_loader_telemetry("Stock Explorer", label, df, cache_state="miss")
    return df


def _prepare_company_history(df: pd.DataFrame) -> pd.DataFrame:
    if df.empty:
        return df
    ...
    if "Date of record" in prepared.columns:
        prepared = prepared.sort_values("Date of record").reset_index(drop=True)

    return prepared


@st.cache_data(ttl=600)
def load_company_history(ticker: str) -> pd.DataFrame:
    normalized_ticker = normalize_ticker(ticker)
    if not normalized_ticker:
        return pd.DataFrame()

    try:
        df = _read_company_history(
            STOCK_EXPLORER_VIEW_QUERY,
            normalized_ticker,
            "load_company_history_view",
        )
    except Exception as exc:
        print(f"[perf] load_company_history_view fallback: {exc}")
        df = _read_company_history(
            STOCK_EXPLORER_BASE_QUERY,
            normalized_ticker,
            "load_company_history_base",
        )

    return _prepare_company_history(df)
